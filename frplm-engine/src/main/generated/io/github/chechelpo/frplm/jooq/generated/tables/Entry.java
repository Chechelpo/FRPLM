...
public class Entry extends TableImpl<EntryRecord> {
    ...
    public UniqueKey<EntryRecord> getPrimaryKey() {
        return Keys.PK_ENTRY;
    }

    @Override
    public List<UniqueKey<EntryRecord>> getUniqueKeys() {
        return Arrays.asList(Keys.UQ_ENTRY_NAME_PER_LOREBOOK);
    }

    @Override
    public List<ForeignKey<EntryRecord, ?>> getReferences() {
        return Arrays.asList(Keys.CONSTRAINT_3, Keys.FK_ENTRY_TO_LOREBOOK);
    }
    ...
    public LorebooksPath lorebooks() {
        if (_lorebooks == null)
            _lorebooks = new LorebooksPath(this, Keys.FK_ENTRY_TO_LOREBOOK, null);
    ...
    public EntryKeywordsPath entryKeywords() {
        if (_entryKeywords == null)
            _entryKeywords = new EntryKeywordsPath(this, null, Keys.FK_ENTRY_KEYWORDS_TO_ENTRY.getInverseKey());

        return _entryKeywords;
    }

    private transient EntryPrologConditionPath _entryPrologCondition;

    /**
     * Get the implicit to-many join path to the
     * <code>PUBLIC.ENTRY_PROLOG_CONDITION</code> table
     */
    public EntryPrologConditionPath entryPrologCondition() {
        if (_entryPrologCondition == null)
            _entryPrologCondition = new EntryPrologConditionPath(this, null, Keys.FK_ENTRY_PROLOG_CONDITION_TO_ENTRY.getInverseKey());
