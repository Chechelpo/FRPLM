...
public class Locations extends TableImpl<LocationsRecord> {
     ...
     */
    public final TableField<LocationsRecord, Integer> X = createField(DSL.name("X"), SQLDataType.INTEGER.nullable(false).defaultValue(DSL.field(DSL.raw("0"), SQLDataType.INTEGER)), this, "");

    /**
     * The column <code>PUBLIC.LOCATIONS.Y</code>.
     */
    public final TableField<LocationsRecord, Integer> Y = createField(DSL.name("Y"), SQLDataType.INTEGER.nullable(false).defaultValue(DSL.field(DSL.raw("0"), SQLDataType.INTEGER)), this, "");
    ...
    public List<UniqueKey<LocationsRecord>> getUniqueKeys() {
        return Arrays.asList(Keys.UNIQUE_LOCATION_NAME_PER_WORLD, Keys.UQ_LOCATION_NAME_PER_REGION);
    }

    @Override
    public List<ForeignKey<LocationsRecord, ?>> getReferences() {
        return Arrays.asList(Keys.CONSTRAINT_5E, Keys.CONSTRAINT_5EB, Keys.FK_LOCATION_TO_REGION, Keys.FK_LOCATION_TO_WORLD);
    }

    private transient WorldsPath _constraint_5e;

    /**
     * Get the implicit join path to the <code>PUBLIC.WORLDS</code> table, via
     * the <code>CONSTRAINT_5E</code> key.
     */
    public WorldsPath constraint_5e() {
        if (_constraint_5e == null)
            _constraint_5e = new WorldsPath(this, Keys.CONSTRAINT_5E, null);

        return _constraint_5e;
    }
    ...
    private transient WorldsPath _fkLocationToWorld;

    /**
     * Get the implicit join path to the <code>PUBLIC.WORLDS</code> table, via
     * the <code>FK_LOCATION_TO_WORLD</code> key.
     */
    public WorldsPath fkLocationToWorld() {
        if (_fkLocationToWorld == null)
            _fkLocationToWorld = new WorldsPath(this, Keys.FK_LOCATION_TO_WORLD, null);

        return _fkLocationToWorld;
    }
