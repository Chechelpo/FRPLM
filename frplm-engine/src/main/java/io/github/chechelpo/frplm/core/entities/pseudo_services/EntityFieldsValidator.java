...
public abstract class EntityFieldsValidator<R extends TableRecord<R>> implements FieldValidator<R>
{
    ...
    /**
     * Validates a data payload by checking each field assignment against a set of allowed fields and validating the assigned value.
     *
     * @param payload         the data payload containing field assignments to validate
     * @param allowedFieldSet the set of fields that are allowed to be present in the payload
    ...
    /**
     * Checks if all required fields from the specified set are present in the given data payload.
     *
     * @param payload          the data payload to check for field assignments
     * @param requiredFieldSet the set of required fields that must be assigned in the payload
     * @return an Optional containing the first required field that is missing from the payload, or an empty Optional if all required fields are present
     */
        ...
        for (Map.Entry<TableField<R, ?>, Object> entry : payload.assignments().entrySet())
            if (keys.contains(entry.getKey()))
                return FieldActionResult.wrongValue(
                        "Non-creation data payload assigns key field " + entry.getKey(),
                        entry.getKey(),
                        entry.getValue(),
                        payload
                );
    ...
    public FieldActionResult<R, EntityDataPayload<R>> validateDataCreationPayload(EntityDataPayload<R> payload) {
        ...
        fieldInfoMap.values()
                .forEach(fieldInfo -> payload.consumeIfAbsent(fieldInfo.getDefaultValue()));
