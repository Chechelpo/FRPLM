...
#### Entries Fields
- **name**: entry's name for easier search, may be null. Implemented
- **content**: text to inject on the condition. Implemented
- **Probability**: standard (0 ~ 100%) probability of the entry being activated if its requirements are met. Implemented
- **Strategy**: how this entry is scanned, with three possible values.
  - Constant: entry is always going to be active when possible. Implemented
  - Normal: entry is activated on keywords match. Implemented
  - Embedding: Normal insertion + embedding comparison to previous messages. Option's there, not implemented yet
    - Embed text: By default the entire entry is embeded for activation, but you can pick a smaller string to embed instead. Implemented (no embedding yet)
- **Conditions** Entries may be from a Character lorebook and as such may be eligible only if the character is present. Not implented, will be tested in prolog.
  - In location: Entry can only be activated if a character is in a particular location. Implemented via the location lorebook
  - Tag: Entry can only be activated if at least one character/location that subscribes to the tag is present. Not implemented
  - Entity present: Same as tag but referring to a single character/location. Not implemented
- **Keywords:** Keywords that activate the entry as per standard lorebook. See Keywords section for clarification on how they work. Implemented
- **Outlet:** Up to now, we only discussed when, now we discuss where. By default, entries are injected to one of our standard ```{{outlet::<name>}}``` but its overridable per entry (see outlets section for clarification.) Implemented.
- **Prevent further recursion:** won't activate other entries. Recursion not yet implemented
- **Non-recursable:** Entry cannot be activated by other entries. Not implemented
- **Delay until recursion:** Entry can only be activated by other entries. Not implemented
- **Delay:** Up to how many turns will this entry be activated. Not implemented
- **Cooldown:** How much time before the entry is available for activation again. Not implemented
- **Sticky:** How many messages will the entry persist. Not implemented
### About keywords
...
- **Travel-time** How much time does the clock advance when moving to the neighbour? Implemented(half, world clock still in the making)
- **description** How the LLM is supposed to introduce the road to the location. Implemented
