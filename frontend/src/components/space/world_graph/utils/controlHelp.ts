...
export const WORLD_GRAPH_CONTROL_HELP_SECTIONS = [
    ...
    },
    {
        ...
        entries: [
            ...
            {control: "location:name / character:name", description: "Find one location by name, or select starting locations for matching characters. Ambiguous matches remain available in the result list."},
            {control: "Match count", description: "Shows the number of matching regions, locations, or characters for the settled query."},
            {control: "↑ / ↓ in results", description: "Move the active choice through a multi-match result list; traversal wraps and keeps the active row in view."},
            {control: "Enter on result", description: "Use the active entity as the exact typed search parameter and apply only that result."},
            {control: "Result click", description: "Apply the clicked result exactly like Enter. The list stays within the graph's lower half and scrolls when needed."},
            {control: "Search / Enter", description: "Run the current query immediately instead of waiting for the debounce when no multi-match result list is open."},
