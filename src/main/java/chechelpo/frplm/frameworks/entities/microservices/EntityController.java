...
import chechelpo.frplm.frameworks.entities.fields.format.Coercer;
...
import static chechelpo.frplm.config.controllers.ControllerPaths.*;
        ...
        >
{
    ...
    private final HashMap<String, Coercer<?>> field_coercers = new HashMap<>();

    private final List<EntityKey<R>> temporarilyUnavailable = new ArrayList<>();
    ...
    void registerPublicField(@NotNull TableField<R,?> field, @Nullable String name, @Nullable Coercer<?> coercer) {
        ...
        if (coercer != null)
            field_coercers.put(name, coercer);
    ...
    protected EntityKey<R> extractKey(@NotNull Map<String, Object> params){
        ...
        for (Map.Entry<String, Object> entry : params.entrySet()) {
            ...
            if (!service.isKey(field)){
                ...
                throw new InvalidKey("Field " + field + " is not a key", Severity.USER);
            }

            builder.unsafeSet(
                    field,
                    field_coercers.get(entry.getKey()).coerce(entry.getValue())
            );
        }
    ...
    protected EntityDataPayload<R> extractPayload(@NotNull Map<String, Object> params){
        HashMap<TableField<R,?>, Object> payload = new HashMap<>(params.size());
        for (Map.Entry<String, Object> entry : params.entrySet()) {
            if (!to_table_field.containsKey(entry.getKey())){
                log.error("Unknown field name {}", entry.getKey());
                throw new UnknownField("Unknown field " + entry.getKey());
            ...
            payload.put(
                    to_table_field.get(entry.getKey()),
                    field_coercers.get(entry.getKey()).coerce(entry.getValue())
            );
        }
